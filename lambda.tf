...
data "aws_iam_policy_document" "lambda-iam-policy" {
  # Allow Lambda logs creation and putting
  statement {
  ...
  statement {
    sid = "Week13LambdaSecretsManager"
    effect = "Allow"
    actions = [
...
resource "aws_lambda_function" "lambda" {
  ...
  vpc_config {
    security_group_ids = [aws_default_security_group.week13-def-sg.id]
    subnet_ids = [aws_subnet.week13-pri-a.id, aws_subnet.week13-pri-b.id]
  }
}


# Create a default security group so that the lambda function has an interface in this VPC
